# Root-Cause Audit Item 26475c1bd0
...
- original_rationale: It converts the star groups to the answer with the wrong mixed-radix formula for `x`, so it expands the `*` blocks into the wrong numbers of `b` characters.
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = n-1; i >= 0; --i) {
            if (s[i] == '*') {
                long long add = x - cnt[i];
                long long reps = min(add, (long long)k);
                result[i] = 'a';
                for (int j = 0; j < reps; ++j) {
                    result.insert(result.begin() + i + 1, 'b');
                }
                x -= reps;
            }
        }
