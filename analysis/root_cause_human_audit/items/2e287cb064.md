# Root-Cause Audit Item 2e287cb064
...
int main() {
    ...
    while (t--) {
        long long a, b, c;
        cin >> a >> b >> c;
        array<long long,3> l = {a,b,c};

        bool ok = false;

        // Try breaking each stick i into (x, l[i]-x) where x in [1, l[i]-1].
        // Rectangle condition: remaining four sides are {p, q, r, s} where we need two pairs.
        // For any rectangle, we can sort the four lengths: let x1<=x2<=x3<=x4.
        // Then it's possible iff x1==x3 and x2==x4 (opposite sides equal).
        //
        // We'll attempt efficient check: Since we have exactly 3 original sticks,
        // after one break we have multiset {l[j], l[k], x, L-x}.
        // To satisfy rectangle condition, the two unbroken sticks must match the two sides
        // in some pairing. That implies the broken parts must be:
        //   L = a1 + a2 where a1,a2 are the unbroken sticks? Actually:
        // If unbroken sticks are p and q, then rectangle requires sides (p,p) and (q,q) in some order,
        // or (p,q) as equal pairs accordingly. Since we only have one p and one q unbroken,
        // the only way is that one of broken parts equals p and the other equals q.
        // So we need to break L into (p, q) in some assignment.
        //
        // Therefore check if for some permutation:
        //   l[i] = l[j] + l[k]  AND break l[i] into exactly those lengths (both > 0).
        // But also we must ensure opposite sides pairs: that will happen with two sticks equal to l[j]
        // and two sticks equal to l[k] using the unbroken stick and one broken piece each.
        //
        // Let unbroken lengths be x and y, and broken lengths be u and v.
        // We already have x and y from unbroken. For rectangle, we need pair equality:
        // {x, y, u, v} must be {m, m, n, n}.
        // The only feasible pairing with exactly one x and one y is:
        //   {u,v} = {x,y} (so each appears twice).
        // That means broken parts must be x and y, hence broken stick length must be x+y.
        //
        // So condition: exists i such that l[i] == l[j] + l[k].
        // Also need x>0 and y>0 automatically given constraints.
        for (int i = 0; i < 3; i++) {
            int j = (i + 1) % 3;
            int k = (i + 2) % 3;
            long long L = l[i], p = l[j], q = l[k];
            if (L == p + q) {
                ok = true;
                break;
            }
