# Root-Cause Audit Item 5554036841
...
int main() {
    ...
    while (t--) {
        ...
        vector<pair<int, int>> sociability;
        for (int i = 0; i < n; ++i) {
            int a;
            cin >> a;
            sociability.push_back({a, i + 1});
        }
        ...
        for (int i = n - 1; i >= 1; --i) {
            if (sociability[i].first == 0) break;
            max_talks += sociability[i].first;
            talks.push_back({sociability[0].second, sociability[i].second});
        }
