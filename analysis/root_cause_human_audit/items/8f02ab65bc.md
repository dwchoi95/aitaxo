# Root-Cause Audit Item 8f02ab65bc
...
- original_rationale: The greedy reassignment inside liked/disliked groups is wrong and does not enforce the required ordering optimally.
...
int main() {
    ...
    while (t--) {
        ...
        std::vector<int> liked_songs, disliked_songs;

        for (int i = 0; i < n; ++i) {
            if (s[i] == '0') {
                disliked_songs.push_back(p[i]);
            } else {
                liked_songs.push_back(p[i]);
            }
        }

        std::sort(liked_songs.begin(), liked_songs.end());
        std::sort(disliked_songs.begin(), disliked_songs.end(), std::greater<int>());

        std::vector<int> q(n);
        int liked_idx = 0, disliked_idx = 0;

        for (int i = 0; i < n; ++i) {
            if (s[i] == '0') {
                q[i] = disliked_songs[disliked_idx++];
            } else {
                q[i] = liked_songs[liked_idx++];
            }
        }
