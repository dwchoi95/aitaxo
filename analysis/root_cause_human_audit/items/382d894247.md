# Root-Cause Audit Item 382d894247
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        if (a[i] <= maxA) pos[a[i]].push_back(i);
    }

    // Compute longest subsequence of indices i1 < i2 < ... < ik such that
    // a[i_t] == t + (something)? Not directly.
    // We need maximum k where there exist k positions in the remaining sequence
    // that are fixed points, i.e., choose indices i1 < i2 < ... < ik from original
    // to become positions p1 < p2 < ... < pk in the resulting sequence,
    // with requirement: a[i_t] == p_t.
    //
    // If we take a subsequence of elements kept, their relative order becomes the
    // new sequence. Let p_t be t plus number of removed elements before the chosen
    // element. But p_t is simply the rank in the kept sequence. Therefore:
    // the fixed points correspond to kept elements whose value equals their rank.
    //
    // Thus we need maximum size k of a subsequence (kept elements) such that for each
    // chosen element, its value equals its position (rank) among all kept elements.
    //
    // Greedy/DP:
    // Let dp[x] = maximum number of fixed points achievable where the current fixed point
    // position is x. Equivalent to count how many fixed points we can get with ranks.
    //
    // Another view:
    // Suppose we keep some subsequence of original indices. Let ranks start at 1.
    // A fixed point at rank r means the element chosen at that rank must have value r.
    // So we need to choose fixed-point elements at ranks r1 < r2 < ... < rk where
    // at rank rt we choose an occurrence of value rt, and between them we may keep
    // arbitrary other elements (which shift ranks).
    //
    // If we decide the ranks of fixed points, we just need to ensure we can pick
    // for each fixed rank rt an occurrence of value rt in increasing index order,
    // while the number of kept elements between fixed ranks equals (rt - previous_rt).
    // That means:
    // - We must keep exactly (rt - prev_rt - 1) elements between the two fixed elements
    //   (any values).
    // - This is always possible as long as there are enough indices in between.
    //
    // This reduces to a longest chain problem on occurrences with feasibil
...[clipped]...
g r,
    // previous occurrences could have index > i. However the condition i-j >= r-p implies
    // i>j automatically when r>p and i-r >= j-p. Yet it could still allow j>i? Let's check:
    // If j>i then i-j negative but r-p positive => i-j >= r-p impossible. So feasibility
    // implies j<=i. Therefore the inequality already enforces correct order; no need to explicitly
    // check j<i.
    //
    // Coordinate compress keys (i-r) over all occurrences.
    vector<long long> allKeys;
    allKeys.reserve(n);
    for (int i = 1; i <= n; i++) {
        allKeys.push_back((long long)i - (long long)a[i]);
    }
    sort(allKeys.begin(), allKeys.end());
    allKeys.erase(unique(allKeys.begin(), allKeys.end()), allKeys.end());

    auto getPos = [&](long long key) {
        return (int)(lower_bound(allKeys.begin(), allKeys.end(), key) - allKeys.begin());
    };

    // Fenwick tree for prefix maximum: query max over keys <= x
    struct FenwickMax {
        int N;
        vector<int> bit;
        FenwickMax(int n=0): N(n), bit(n+1, 0) {}
        void update(int idx, int val) {
            for (int i = idx+1; i <= N; i += i&-i) bit[i] = max(bit[i], val);
        }
        int query(int idx) { // max on [0..idx]
            int res = 0;
            for (int i = idx+1; i > 0; i -= i&-i) res = max(res, bit[i]);
            return res;
        }
    };

    FenwickMax fw((int)allKeys.size());
    int ans = 0;

    // Process values r in increasing order; within same r, transitions not allowed because fixed points
    // require previous value p < r (strict). So we must avoid using updates from the same r for computing others.
    // We'll compute dp for all occurrences of value r first, store and then update.
    for (int r = 1; r <= maxA; r++) {
        if (pos[r].empty()) continue;
        vector<pair<int,int>> updates; // (keyPos, dpVal)
        updates.reserve(pos[r].size());

        for (int idx : pos[r]) {
            long long key = (long long)idx - (long long)r;
            int keyPos = getPos(key);
            int bestPrev = fw.query(keyPos);
            int dp = bestPrev + 1;
            updates.push_back({keyPos, dp});
            ans = max(ans, dp);
        }

        // Apply updates after finishing this r
        for (auto [keyPos, dpVal] : updates) {
            fw.update(keyPos, dpVal);
        }
    }

    // Also possible to have fixed points count 0? But we want maximum, and can always keep none => f=0.
    cout << ans << "\n";
    return 0;
}
```
