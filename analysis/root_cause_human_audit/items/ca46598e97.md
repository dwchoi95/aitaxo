# Root-Cause Audit Item ca46598e97
...
int main() {
    ...
    // Sort by skill ascending; for same skill, sort by neatness ascending.
    ...
    // Greedy:
    // Maintain a set of climbable alpinists by their neatness (a).
    // Take among them the one with largest neatness? Actually choose smallest neatness
    // to keep mountain difficulty low for future. But constraints allow a only increases
    // difficulty to max(cur, a). Therefore choosing smaller a when cur is already high
    // is always better/equal.
    //
    // We can model process:
    // At any time, climber with skill s can be chosen only if s >= cur.
    // After climbing, cur becomes max(cur, a).
    //
    // If we always choose the available climber with smallest a,
    // cur after each step is minimized, thus maximizing count.
    //
    // We'll use a min-heap of a for all alpinists with s >= cur.
    // However cur changes; easier:
    // Sort by s descending, process while selecting smallest a among those with s >= cur.
    // We'll instead sort by s descending and use a pointer to push into heap based on s >= cur.
    // But cur changes upward, so set of eligible alpinists only shrinks (since requirement s >= cur becomes harder).
    // Using s ascending would expand eligibility as cur decreases; but cur never decreases.
    // Thus eligibility expands? Requirement is s >= cur; as cur increases, some become ineligible.
    // That suggests processing from high cur downwards isn't straightforward.
    //
    // Alternative known solution:
    // Sort by s ascending. For each climber, if cur <= s_i they can climb.
    // After choosing, cur becomes max(cur, a_i).
    // We can maximize count by dynamic strategy:
    // Split into two groups by whether a_i <= s_i etc? Not.
    //
    // Correct approach:
    // Sort by s ascending, and maintain minimal possible cur for each possible count?
    // But we only need max count: can be solved by:
    // Greedy selecting alpinists in order of increasing a among 
...[clipped]...
h ability to discard newly ineligible elements:
    // If we keep all chosen? Actually once chosen, they are removed.
    // We need to repeatedly pick smallest a among remaining with s >= cur.
    //
    // Since cur only increases, the condition s >= cur becomes stricter.
    // We can pre-sort by s, and use a pointer from high s downward:
    // Maintain index over sorted by s descending; keep a min-heap of a for alpinists with s >= cur.
    // As cur increases, fewer satisfy s >= cur, but our heap may contain elements with s < new cur.
    // However if we add only those with s >= current cur at the moment, later when cur increases we must remove those that no longer satisfy.
    //
    // To handle that, store in heap elements (a, s). Pop from heap while s < cur (they became ineligible).
    //
    // Also we must ensure we have pushed all candidates that could ever become eligible:
    // We only push those with s >= current cur (current threshold).
    // When cur increases, some remaining have s < new threshold, so they won't be pushed anyway.
    // But we might have already pushed them under old threshold; we will discard them when they fail.
    //
    // We can do:
    // Sort by s descending.
    // Keep pointer p over array, pushing into heap all alpinists with s >= cur at each step.
    // Then choose from heap the one with minimal a; if heap top has s < cur, discard until valid.
    // After selecting, cur = max(cur, a).
    //
    // This is correct because among all eligible alpinists, choosing one with minimal a minimizes
    // resulting cur (since cur' = max(cur, a)). With minimal cur', eligibility for the next step is maximized.
    //
    // Complexity: O(n log n).

    vector<pair<long long,long long>> w = v;
    sort(w.begin(), w.end(), [](const auto& x, const auto& y){
        return x.first > y.first; // s descending
    });

    // min-heap by a; store (a, s)
    using T = tuple<long long,long long>; // (a, s)
    priority_queue<T, vector<T>, greater<T>> pq;

    int p = 0;
    int picked = 0;
    while (true) {
        while (p < n && w[p].first >= cur) {
            pq.emplace(w[p].second, w[p].first);
            ++p;
        }
        while (!pq.empty() && get<1>(pq.top()) < cur) pq.pop();
        if (pq.empty()) break;

        auto [a, s] = pq.top();
        pq.pop();
        // s >= cur guaranteed by cleanup, so climb possible.
        cur = max(cur, a);
        ++picked;
    }
