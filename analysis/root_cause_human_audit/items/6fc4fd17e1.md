# Root-Cause Audit Item 6fc4fd17e1
...
static inline bool possible(long long a, long long b, long long c) {
    vector<long long> s = {a, b, c};
    sort(s.begin(), s.end()); // s[2] is max
    long long x = s[0], y = s[1], z = s[2]; // z = max

    // We will break z into p and q where p+q=z and p,q>0 integers.
    // Need p with one of x,y as opposite sides, and q with the other:
    // i.e., rectangle sides are (p, x) and (q, y) in some order, so:
    // p must equal y and q must equal x, OR p must equal x and q must equal y,
    // but we only need existence of any split that makes two distinct pairs equal
    // because rectangle sides must be two lengths: (u and v) with total sticks:
    // must be {u,u,v,v} from the four sticks.
    //
    // Here, only way with three given lengths where one is split:
    // since x and y are existing unbroken sticks, to get {u,u,v,v},
    // x and y must each match one of the split parts (possibly both to same value).
    //
    // So we consider whether splitting z can produce:
    // 1) x and y as the two parts (p=x,q=y) -> requires x+y=z.
    // 2) 2x with z part? Actually to get {x,x, x, x} would need x+x = z and y=x.
    // General condition becomes:
    // split z into parts whose multiset equals {x,x} and y==x (square using x,x),
    // or equals {y,y} and x==y,
    // or equals {x,y}.
    //
    // All covered by checking: x+y==z OR 2*x==z (and y==x) OR 2*y==z (and x==y).
    //
    // But more directly for Codeforces known solution:
    // Only possible if max <= sum of others and moreover by splitting max to match both others.
    // Here because the rectangle requires two equal pairs:
    // after sorting, we can assume x<=y<=z:
    // Need z to be broken into two integers that are exactly x and y (then opposite sides x and y),
    // or into two equal integers equal to x if y==x, or equal to y if x==y (same).
    return (x + y == z) || (2*x == z && y == x) || (2*y == z && x == y);
}
