# Root-Cause Audit Item 310774c8ab
...
static inline bool isLeadingZero(const string &t) {
    // t is full digit string (length L <= 8), integer has no sign
    // Leading zero defined as 0 digits before first nonzero digit.
    // Exception: integer zero (all digits are 0) has no leading zero.
    bool allZero = true;
    for (char c : t) if (c != '0') { allZero = false; break; }
    if (allZero) return false; // "0" has no leading zero, but "0000" does: however
    // We will treat leading zeros as any zero digits before first nonzero.
    // For allZero case, we must disallow if length > 1 (as in examples like "0000").
    // That matches "0000 has three leading zeroes".
    // So: if all digits are zero and length > 1 => has leading zeros => invalid.
    return t.size() > 1; // length>1 => leading zeros exist
}
...
int main() {
    ...
    // digit string length n, must satisfy divisibility by 25 => last two digits in {00,25,50,75}
    vector<pair<char,char>> endings = {{'0','0'},{'2','5'},{'5','0'},{'7','5'}};

    // Collect positions of X and count distinct constraints per candidate assignment.
    // We'll just brute force digits for X (0..9) and assign underscores per last-two requirements + fill rest arbitrarily.
    // But we must count all possibilities, not enumerate all digits.
    // Approach: for each digit dX assigned to X, turn X to fixed digit and for each ending pattern,
    // count ways to fill remaining underscores such that:
    // - last two positions match pattern (compatible with fixed digits/underscores)
    // - no leading zero in final number string.
    // Then sum.
    long long ans = 0;

    // Precompute underscore counts and compatibility per position given chosen X digit.
    for (int dx = 0; dx <= 9; dx++) {
        vector<char> fixed(n, 0); // 0 means not fixed, otherwise digit char
        for (int i = 0; i < n; i++) {
            if (s[i] == 'X') fixed[i] = char('0' + dx);
        }

        // For each possible ending (a,b) representing digits of last two positions
        for (auto [a, b] : endings) {
            vector<char> val(n, 0); // 0 means unknown yet, otherwise digit char
            // set fixed X digits
            for (int i = 0; i < n; i++) if (fixed[i]) val[i] = fixed[i];

            // enforce ending digits at positions n-2 and n-1 if n>=2 else divisibility by 25 impossible 
...[clipped]...
 zero.
                // Wait: if first digit is '0' but later digits include non-zero => leading zero exists => invalid.
                // If first digit is '0' and all digits are zero => valid only when n==1; for n>1 invalid (as per statement).
                // Therefore for n>1: valid iff first digit != '0'.
                // For n==1: valid iff digit can be 0 (integer zero).
                if (n == 1) {
                    // Not used currently in ending loop, but keep safe.
                } else {
                    char firstChar = val[0];
                    // Determine number of assignments for remaining underscores where first digit is not zero.
                    // We only need to handle whether position 0 is fixed or free underscore.
                    // If position 0 is fixed digit:
                    if (firstChar != 0) {
                        if (firstChar == '0') continue; // invalid
                        ans += (long long)pow(10, freeUnderscores); // all free underscores can be any digit
                    } else {
                        // s[0] must be '_' because it's not X and not literal digit.
                        // Then first digit cannot be zero => choices 1..9 => 9 choices.
                        // After fixing first digit, remaining freeUnderscores-1 underscores free: each 10 choices.
                        if (freeUnderscores == 0) continue; // shouldn't happen
                        ans += (long long)9 * (long long)pow(10, freeUnderscores - 1);
                    }
                }
            }
        }
    ...
    if (n == 1) {
        // divisible by 25 only possible integer is 0
        char ch = s[0];
        if (ch == '0') ans = 1;
        else if (ch == '_') ans = 1;      // '_' can be 0
        else if (ch == 'X') ans = 1;      // X can be 0
        else ans = 0;                     // digits 1..9
    } else {
        // For n>=2, we used leading zero rule as first digit != 0.
        // But need to ensure this is correct with the problem's definition:
        // For n>=2, any string starting with '0' has leading zero(s) unless all digits are zero.
        // Could all digits be zero and still satisfy divisibility by 25? It would be divisible (000..0 ends with 00).
        // But it's invalid per statement (e.g., "0000 has three leading zeroes").
        // Since first digit would be 0, we excluded it already; good.
    }
