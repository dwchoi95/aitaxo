# Root-Cause Audit Item 5c19aec84c
...
int main() {
    ...
    double current_division = 1;

    for (int i = 0, j = 0; i < n; ++i) {
        if (b[i] == '*') {
            current_product *= a[i];
            ans += count[current_product / current_division]++;
        } else {
            current_division *= a[i];
            ans += count[current_product / current_division]++;
        }

        while (j < i && (current_product != static_cast<int>(current_product) || current_division != static_cast<int>(current_division))) {
            if (b[j] == '*') {
                count[current_product / current_division]--;
                current_product /= a[j];
            } else {
                count[current_product / current_division]--;
                current_division /= a[j];
            }
            ++j;
        }
    }
