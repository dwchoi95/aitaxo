# Root-Cause Audit Item 93162f7967
...
int main() {
    ...
    while (t--) {
        ...
        // If k <= 1, need diameter < k-1 <= 0, impossible unless n=1 and m=0
        // For n=1, diameter = 0. Condition: 0 < k-1
        if (k <= 1) {
            if (n == 1 && m == 0) cout << "YES\n";
            else cout << "NO\n";
            continue;
        ...
        // Diameter < k-1  <=>  graph has diameter <= k-2.
        long long D = k - 2;

        // If n==1: only possible graph has m=0 and diameter=0 which satisfies for any k>=2
        if (n == 1) {
            cout << (m == 0 ? "YES\n" : "NO\n");
            continue;
        }
        ...
        // Need existence of some connected simple graph on n vertices with diameter <= D
        // and exactly m edges.
        //
        // Minimum edges for connected graph is n-1 (tree).
        // If m < n-1 => impossible.
        if (m < n - 1) {
            cout << "NO\n";
            continue;
        ...
        // For diameter constraint:
        // - If D == 0: requires diameter <=0 but n>1 => impossible.
        if (D == 0) {
            cout << "NO\n";
            continue;
        }

        // For D >= 1, we can construct a connected graph with diameter <= D:
        // It's always possible for any n when D >= 1? Not always: D=1 implies diameter<=1 => complete graph.
        // So we handle D==1 separately: must be complete graph -> edges = maxEdges.
        if (D == 1) {
            cout << (m == maxEdges ? "YES\n" : "NO\n");
            continue;
        }

        // For D >= 2, we can realize any number of edges between (n-1) and maxEdges
        // while keeping diameter <= D.
        //
        // Construction idea:
        // - Start with a star (diameter=2) which is <= D for D>=2. Has n-1 edges.
        // - Add edges arbitrarily between leaves; adding edges cannot increase diameter.
        // Hence any m in [n-1, maxEdges] is achievable.
        cout << "YES\n";
    }
    return 0;
}
