# Root-Cause Audit Item 8c5af78403
...
int main() {
    ...
    double min_dist = dist(points[K - 1], points[K % N]) + dist(points[(K - 2 + N) % N], points[K % N]);
    double path_length = ans - min_dist + dist(points[K - 1], points[(K - 2 + N) % N]);
    
    cout << fixed << setprecision(6) << path_length << endl;
