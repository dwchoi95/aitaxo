# Root-Cause Audit Item 87dbea4311
...
vector<int> red_count, blue_count;
int k;
...
void dfs(int v, int parent) {
    ...
    for (int u : adj[v]) {
        if (u != parent) {
            dfs(u, v);
            red_count[v] += red_count[u];
            blue_count[v] += blue_count[u];
        }
    }
    blue_count[v] = min(blue_count[v], k - red_count[v]);
}
...
int main() {
    ...
    adj.resize(n + 1);
    red_count.resize(n + 1, 0);
    blue_count.resize(n + 1, 0);
    ...
    long long result = 0;
    for (int i = 1; i <= n; ++i) {
        result += 1LL * (red_count[i] - blue_count[i]) * (n - red_count[i]);
    }
