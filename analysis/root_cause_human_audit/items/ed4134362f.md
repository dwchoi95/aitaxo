# Root-Cause Audit Item ed4134362f
...
- original_rationale: It makes a local residue-based greedy choice for how many 1/2/3-coin operations to use, but that greedy rule is not globally optimal across all array elements.
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; i++) {
            long long x = a[i];
            // minimal number of coins using 1,2,3 to sum to x:
            // Use as many 3's as possible; handle remainder.
            long long res;
            if (x % 3 == 0) res = x / 3;
            else if (x % 3 == 1) res = (x - 4) / 3 + 2; // 3k + 1 = 3(k-1)+2+2 (for x>=4)
            else res = (x - 2) / 3 + 1; // 3k + 2 = 3k + 2
            // For x=1 or 2 special cases are covered by formulas:
            // x=1 -> (1-4)/3 +2 = (-3)/3+2 =1
            // x=2 -> (2-2)/3+1 = 0+1=1
            ans = max(ans, res);
        }
