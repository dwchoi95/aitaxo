# Root-Cause Audit Item ffa8f1202b
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < k; ++i) {
            int y;
            cin >> y;
            verticalPoints.push_back(y);
        }

        // Find the largest difference in horizontalPoints
        int maxHorizontalDiff = 0;
        for (int i = 1; i < horizontalPoints.size(); ++i) {
            maxHorizontalDiff = max(maxHorizontalDiff, horizontalPoints[i] - horizontalPoints[i-1]);
        }

        // Find the largest difference in verticalPoints
        int maxVerticalDiff = 0;
        for (int i = 1; i < verticalPoints.size(); ++i) {
            maxVerticalDiff = max(maxVerticalDiff, verticalPoints[i] - verticalPoints[i-1]);
        }

        // The doubled maximum area will be the product of the two largest differences
        cout << (long long)maxHorizontalDiff * maxVerticalDiff << endl;
    }
