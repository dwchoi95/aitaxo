# Root-Cause Audit Item f1dbe3f7a9
...
- original_labels: GE1.2,GE4.1
- original_rationale: Starts with 1 so the divisibility condition already fails, and the repunit construction overflows long long for large n.
...
int main() {
    ...
    while (t--) {
        int n;
        cin >> n;
        // Use numbers like 111...1 (k times). Let a_i be the number with i digits all 1.
        // Then for i>j, a_i mod a_j != 0 (since a_j is odd and any multiple would imply a divisibility
        // pattern that doesn't hold for these repunit numbers for i>j in general).
        // This construction is a known accepted approach for this problem.
        vector<long long> a(n);
        for (int i = 0; i < n; i++) {
            long long x = 0;
            for (int k = 0; k <= i; k++) x = x * 10 + 1; // i+1 ones
            a[i] = x;
        }
