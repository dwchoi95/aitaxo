# Root-Cause Audit Item d282c600b6
...
int main() {
    ...
    int maxClimbers = 0;
    for (int i = 0; i < n; ++i) {
        if (alpinists[i].first >= d || alpinists[i].second >= d) {
            ++maxClimbers;
        } else if (alpinists[i].first < d && alpinists[i].second < d && alpinists[i].first < alpinists[i].second) {
            d = alpinists[i].second;
        }
    }

    cout << maxClimbers << endl;
