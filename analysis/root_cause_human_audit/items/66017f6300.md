# Root-Cause Audit Item 66017f6300
...
int countDivisibleBy25(string s) {
    ...
    do {
        if (s.back() == '5' || s.back() == '0') {
            string num;
            for (int i = 0; i < n; ++i) {
                if (s[i] == 'X') num.push_back(s.back());
                else if (s[i] != '_') num.push_back(s[i]);
            }

            if (num[0] == '0' && count(num.begin(), num.end(), '0') > 1) continue;
