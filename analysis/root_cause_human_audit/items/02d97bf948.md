# Root-Cause Audit Item 02d97bf948
...
int main() {
    ...
    for(int i=n-1;i>=0;i--){
        int v = order[i];
        int s=1;
        for(int to: children[v]) s += sub[to];
        sub[v]=s;
    }

    // DP:
    // Let F[x] = maximum value of (r - b) for a fixed number of red nodes r = x?
    // But we need optimal score: w*(r-b), where w = n - r - b.
    //
    // We'll derive standard solution:
    // Red chooses a union of disjoint connected subtrees in rooted tree (equivalently an "upper-closed" set of red nodes?).
    // Blue may color any node not in red, but only by subtrees that avoid red nodes -> blue's colored nodes form a union of subtrees
    // fully contained in the remaining forest after removing red nodes, so blue will color all nodes available because no limit.
    //
    // For any red coloring set R, Blue will color all nodes that are in components of the forest after deleting R that do not
    // contain a red node (they can't) -> actually any node not red is "available". Blue can color every node not red by choosing
    // for each maximal subtree in the remaining forest whose root is a child of a red node or the original root if not red.
    // Therefore Blue will color ALL non-red nodes that are not "blocked" by red in a way?
    //
    // Wait: Blue cannot choose a subtree that contains a red node. So he can color any node as long as his chosen subtree stays within
    // the maximal red-free connected region (in rooted sense). But since operations can be repeated, Blue can color all nodes in
    // the entire red-free forest. In a tree, removing red nodes splits into components; each component is a subtree in the rooted
    // forest (s
...[clipped]...
/ Option2: do not choose v => combine children selections:
    // If at least one child has non-empty selection, then v becomes activated (+1).
    // If all children are empty, dp_v[0]=0 (no sources => no activated nodes).
    //
    // Combine via knapsack maintaining best activated sum.
    const int NEG = -1e9;
    vector<vector<int>> dp(n+1); // dp sizes up to min(k, sub[v])
    vector<vector<int>> ndp(n+1); // not used
    ...
    for(int v: post){
        int vmax = min(k, sub[v]);
        vector<int> cur(vmax+1, NEG);
        cur[0] = 0;
        // choose v as a source
        if(sub[v] <= k) cur[sub[v]] = sub[v];

        // combine children for option2
        // We'll create knapsack over children where dp means max activated nodes excluding possible +1 for v itself.
        // We'll handle activation of v: if any child non-empty, add 1.
        vector<int> base(vmax+1, NEG);
        base[0]=0;

        for(int to: children[v]){
            int ssz = min(k, sub[to]);
            vector<int> nxt(vmax+1, NEG);
            for(int i=0;i<=vmax;i++){
                if(base[i]==NEG) continue;
                // child empty
                nxt[i] = max(nxt[i], base[i]);
                // child non-empty
                for(int j=1;j<=min(ssz, vmax-i);j++){
                    if(dp[to].size()<= (size_t)j) break;
                    if(dp[to][j]==NEG) continue;
                    nxt[i+j] = max(nxt[i+j], base[i] + dp[to][j]);
                }
            }
            base.swap(nxt);
        }
        // base includes activated nodes inside children but not whether v is activated.
        // If total red >0 => v is activated (+1) unless v itself chosen as source already handled by cur option1.
        // However in option2, v is not chosen as source, so for any c>0 computed from children, v becomes activated.
        // Add +1 for c>0.
        for(int c=1;c<=vmax;c++){
            if(base[c]==NEG) continue;
            cur[c] = max(cur[c], base[c] + 1);
        }
        dp[v].swap(cur);
    }
    ...
    int ans = LLONG_MIN;
    int w_best = 0;
    for(int s=0;s<=k && s<(int)dp[1].size();s++){
        if(dp[1][s]==NEG) continue;
        int w = dp[1][s];
        int r = s;
        int b = n - w; // non-activated nodes are blue
        int score = w * (r - b);
        ans = max(ans, score);
    }
