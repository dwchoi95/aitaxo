# Root-Cause Audit Item d8b34394ae
...
int main() {
    ...
    while (t--) {
        ...
        // Choose x as larger half, y as smaller half.
        // For x = sorted[n-1-i], y = sorted[i]:
        // Since y is small and distinct, x % y < y, thus remainder cannot equal any y in smaller half.
        // But could equal some other element in a (which are all >= y0 maybe smaller).
        // We avoid this by ensuring remainder < smallest of larger-half elements? Not enough.
        // Better strategy:
        // Pair each x with y = smallest element among chosen smaller half such that
        // remainders are exactly y (impossible since remainder < y) so can't be in y set; still
        // might be some other element less than y.
        //
        // Known CF solution:
        // Let’s pair elements by taking i-th smallest as y and i-th from the end as x,
        // and use that for i < n/2: x % y < y <= y_i, and since all elements are distinct,
        // at most one remainder can collide in set; but requirement is just that remainder is not present.
        // The official approach is to use y as the i-th smallest and x as the corresponding element,
        // then check remainder existence and swap if needed. However we need guaranteed construction.
        //
        // Actually there's a simple guaranteed construction:
        // Let S be all elements in a. For each x in upper half, pick y as the smallest element in S.
        // Then x % y is in [0, y-1]. Because all elements in a are positive distinct,
        // if y is the smallest positive element, then x % y is either 0 or < y and hence not in a.
        // Since 0 isn't positive, it also isn't in a. Thus x%y never appears.
        // But we need multiple pairs with distinct (x,y) pairs, and x and y can repeat.
        // Using fixed y_min and different x gives floor(n/2) pairs.
        //
        // Need x != y, so x can't be y_min: choose x from elements excluding y_min.
        // There are n-1 candidates; floor(n/2) <= n-1 for n>=2 holds.
        // So choose y=y_min and x as the largest floor(n/2) elements.
        int y_min = *min_element(a.begin(), a.end());

        // pick x as the largest k elements excluding y_min (if y_min is among them, skip)
        int k = n / 2;
        vector<int> xs;
        sorted = a;
        sort(sorted.begin(), sorted.end());
        for (int i = n - 1; i >= 0 && (int)xs.size() < k; i--) {
            if (sorted[i] != y_min) xs.push_back(sorted[i]);
        }

        // Build pairs (x, y_min)
        // For any such x, remainder r = x % y_min < y_min.
        // Since y_min is the minimum positive in a, no positive element in a is < y_min, and r>0 would imply r in a,
        // but r is either 0 or >=1 and < y_min; in either case, r is not in a.
        // Therefore each pair is valid.
        for (int x : xs) {
            cout << x << ' ' << y_min << "\n";
        }
    }
