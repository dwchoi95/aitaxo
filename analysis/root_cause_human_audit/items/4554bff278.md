# Root-Cause Audit Item 4554bff278
...
- original_rationale: It commits to a local largest-first pairing strategy for talks, but that greedy choice is not a justified construction for all maximum schedules.
...
int main() {
    ...
    while (t--) {
        ...
        // Build list of people by sociability.
        // We can model talks as edges between two currently available people.
        // For maximizing total number of talks, we should use as many talks as possible:
        // Each talk consumes 1 unit from two people, but in fact the meeting ends when
        // a person reaches its limit and leaves. Equivalent optimum:
        // If max(ai) > sum - max(ai), we are limited by sum - max(ai); else by sum/2.
        int mx = 0, idx = -1;
        for (int i = 1; i <= n; i++) {
            if (a[i] > mx) { mx = a[i]; idx = i; }
        }
        long long other = sum - mx;
        long long maxTalks = min(other, sum / 2); // maximal number of talks.

        cout << maxTalks << "\n";
        if (maxTalks == 0) continue;

        // Greedy construction:
        // Repeatedly pair the currently most sociable person with another available person.
        // We'll maintain a max-heap of (remaining, id).
        priority_queue<pair<int,int>> pq;
        for (int i = 1; i <= n; i++) {
            if (a[i] > 0) pq.push({a[i], i});
        }

        // We'll also keep the schedule by always selecting two people with remaining > 0.
        // But note: for maximal talk count, we may not need to use all available capacity,
        // only up to maxTalks.
        // Greedy of picking top two each time achieves optimality for this constraint.
        vector<pair<int,int>> ans;
        ans.reserve((size_t)maxTalks);

        while (!pq.empty() && ans.size() < (size_t)maxTalks) {
            auto [c1, u] = pq.top(); pq.pop();
            if (pq.empty()) break; // can't pair
            auto [c2, v] = pq.top(); pq.pop();

            // Add one talk between u and v.
            ans.push_back({u, v});

            c1--; c2--;
            if (c1 > 0) pq.push({c1, u});
            if (c2 > 0) pq.push({c2, v});
        }

        // Output constructed answers.
        for (auto &pr : ans) {
