# Root-Cause Audit Item faff8f1893
...
int main() {
    ...
    const int MAXA = 500;
    const int LIM = 1024; // XOR values possible within [0..1023] since ai<=500 < 512

    vector<int> cnt(MAXA + 1, 0);
    for (int v : a) cnt[v]++;

    // Collect unique values in increasing order
    vector<int> vals;
    vals.reserve(MAXA + 1);
    for (int v = 0; v <= MAXA; v++) if (cnt[v]) vals.push_back(v);

    // dp[v][mask] where v is current value threshold, but we only need rolling:
    // Standard solution: maintain reachable XORs using increasing subsequence.
    // Because values are <= 500, we can process values in increasing order; within the same value,
    // we can take at most one occurrence because subsequence must be strictly increasing.
    vector<char> dp(LIM, 0), ndp(LIM, 0);
    dp[0] = 1; // empty subsequence

    for (int x : vals) {
        ndp = dp; // not take this value
        for (int mask = 0; mask < LIM; mask++) {
            if (!dp[mask]) continue;
            ndp[mask ^ x] = 1; // take this value (since it's strictly increasing by value)
        }
        dp.swap(ndp);
    }

    vector<int> ans;
    for (int v = 0; v < LIM; v++) if (dp[v]) ans.push_back(v);
