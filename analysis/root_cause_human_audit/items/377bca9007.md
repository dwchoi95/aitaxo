# Root-Cause Audit Item 377bca9007
...
int main() {
    ...
    while (t--) {
        ...
        // The segments are:
        // [l1, r1] length k => r1 = l1 + k - 1
        // [l2, r2] length k-1
        // ...
        // [lk, rk] length 1
        // Non-overlapping and ordered means all segments occur left-to-right.
        //
        // Key observation (standard for this CF problem):
        // We can process suffixes starting from the end and greedily decide feasibility for k
        // by always taking the shortest last segment first.
        //
        // Equivalent DP/greedy:
        // Let S_j be sum of chosen segment of length j.
        // We require S_k < S_{k-1} < ... < S_1? Actually sums are increasing from 1st to k-th:
        // sum(len k) < sum(len k-1) < ... < sum(len 1).
        //
        // If we build from the rightmost (length 1) backwards, we would need sums to be
        // strictly decreasing when considering from right to left.
        //
        // However, there is a much simpler solution used for this problem:
        // Maximum k is the largest such that we can choose k segments from left to right
        // with lengths k..1 and strictly increasing sums.
        // The optimal strategy is to take segments as far left as possible while maintaining the
        // increasing constraint, which can be checked greedily with a binary search on k.
        //
        // We'll implement feasibility check for a given k in O(n) using greedy + prefix sums.
        //
        // Feasibility approach:
        // Choose segments lengths: k, k-1, ..., 1 from left to right.
        // We'll attempt to position them greedily ensuring sums strictly increase.
        //
        // Use for each length len: the segment sum is determined by its start.
        // Greedy chooses the earliest start possible that makes the sum > previous_sum.
        //
        // To do this efficiently, for a fixed len and required condition sum > prev,
        // we need to find minimal index start >= current_pos such that
        // pref[start+len-1] - pref[start-1] > prev.
        //
        // Since a_i > 0, segment sums grow with moving start righ
...[clipped]...

        // end_i = start_i + (k-i) - 1
        //
        // We'll precompute prefix sums, so each segment sum is O(1).
        // Then for a given s, we check increasing in O(k).
        // But doing this for all s is O(n*k) ~ 4.5e7 per test? ok with sum n<=1e5 but need careful.
        //
        // We'll do binary search: log n ~ 17, multiplying becomes too big.
        // Instead compute k directly by trying k from 1 upward and stopping at first failure? Not monotone? likely monotone:
        // If k is possible then k-1 is possible. So binary search is valid.
        //
        // We'll keep binary search but optimize check:
        // For given k, k<=447. We'll for each s check increasing and break early on failure.
        // Worst-case still heavy but acceptable with sum n<=1e5 and k small.
        //
        // Let's proceed.
        ...
        auto feasible = [&](int k)->bool{
            ...
            int maxStart = (int)(n - total + 1);

            // Precompute start offsets for each segment index i (0..k-1)
            vector<int> offset(k,0);
            for(int i=0;i<k;i++){
                // offset of start of segment i relative to s
                // segment i length = k-i
                // sum of lengths of previous segments = i*(2k-i+1)/2
                offset[i] = (int)(1LL*i*(2*k - i + 1)/2);
            }

            for(int s=1; s<=maxStart; s++){
                long long prev = -1; // since sums are positive, -1 works as -inf
                bool ok = true;
                for(int i=0;i<k;i++){
                    int len = k - i;
                    int l = s + offset[i];
        ...
        int lo = 0, hi = 1;
        while(1LL*hi*(hi+1)/2 <= n) hi++;
        hi--; // maximum possible by length
