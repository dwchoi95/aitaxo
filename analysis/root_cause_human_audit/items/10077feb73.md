# Root-Cause Audit Item 10077feb73
...
int main() {
    ...
    while (t--) {
        ...
        // For each residue mod 3, we need the minimal coin count
        // to be able to pay exactly any cost with that residue.
        // Since coins are {1,2,3}, the optimal way to minimize number of coins
        // for a given amount x is:
        // x=3k -> k coins of 3
        // x=3k+1 -> k coins of 3 + one 1 (k+1 coins)
        // x=3k+2 -> k coins of 3 + one 2 (k+1 coins)
        // But we need a fixed multiset for all flavors.
        // It's sufficient to ensure for each residue r, we have enough total
        // number of coins to represent the maximum amount among flavors with that residue,
        // because representation uses only coins of value 3 plus exactly one 1 or 2 depending on residue.
        //
        // Approach:
        // Let best[r] = maximum a_i such that a_i % 3 == r.
        // Then minimum total coin count is sum over residues:
        // for best[0]=3k => need k coins of 3
        // for best[1]=3k+1 => need k coins of 3 and one 1 (k+1 coins, but includes one 1)
        // for best[2]=3k+2 => need k coins of 3 and one 2 (k+1 coins)
        //
        // However coins of value 3 are shared across all residues, so we must take max k
        // across residues that require coins of 3.
        //
        // Since residues 0 uses only 3s, residue 1/2 uses k 3s plus one extra (1 or 2).
        // To cover all three residues, we need:
        // coins of 3: max over k among all residues.
        // coins of 1: 1 if any residue 1 exists (because only need at most one 1)
        // coins of 2: 1 if any residue 2 exists.
        //
        // More carefully:
        // For residue r:
        // amount = 3k + r (where r=0,1,2; for r=0, r means 3k).
        // It can be paid using exactly k coins of 3 and:
        // - r=0: nothing extra
        // - r=1: one coin of 1
        // - r=2: one coin of 2
        // (because using more 1/2 would only increase coin count and also could increase needed totals).
        //
        // With a multiset, for each residue we just need enough 3-coins to cover that residue's maximum k.
        // Since all payments use only 3-coins in that part, max k suffices.
        //
        // Also need at most one coin of 1 or 2 because any amount with residue 1 can be represented
        // with exactly one 1 plus appropriate 3s; similarly residue 2 with one 2.
        //
        // Therefore:
        // let maxk = max over i of floor(a_i/3)
        // But floor(a_i/3) equals k for residue 0,1,2 (for residue 1/2 also floor is k).
        // Coins of 3 needed is max k among all amounts.
        // Coins of 1 needed is 1 iff any a_i % 3 == 1
        // Coins of 2 needed is 1 iff any a_i % 3 == 2
        ...
        long long ans = maxk + (need1 ? 1 : 0) + (need2 ? 1 : 0);
        cout << ans << "\n";
    }
    return 0;
}
