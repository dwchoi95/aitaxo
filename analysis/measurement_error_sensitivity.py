...
def main() -> None:
    ...
    lines = [
        ...
        r"\begin{table}[t]",
        r"\centering",
        rf"\caption{{Judge measurement-error sensitivity. P/R use the {len(agree)} gold items with annotator-agreed primary families. Corrected deltas subtract each arm's judge-minus-human bias measured on the 375-item gold.}}",
        r"\label{tab:measerr}",
        r"\scriptsize",
    ...
    for fam in FOCUS:
        ...
        lines.append(
            ...
            f"{pr} & "
            f"{dp['raw_delta'] * 100:+.1f}$\\to${dp['corrected_delta'] * 100:+.1f} & "
            f"{dm['raw_delta'] * 100:+.1f}$\\to${dm['corrected_delta'] * 100:+.1f} \\\\"
        )
    ...
    lines += [
        r"\midrule",
        f"AE1+GE1 & -- & {md_raw_delta * 100:+.1f}$\\to${md_corr_delta * 100:+.1f} & "
        f"{md_mod_raw_delta * 100:+.1f}$\\to${md_mod_corr_delta * 100:+.1f} \\\\",
        r"\bottomrule",
        r"\end{tabular}",
        r"\end{table}",
        "",
