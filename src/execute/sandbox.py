...
class Sandbox:
    ...
    # outside the per-run scratch dir; rlimits bound CPU and output file size; the child runs in
    # its own session so a fork bomb is killed as a process group at the wall-clock timeout;
    # output is capped; execution is non-root. See ASSUMPTIONS.md.
    ...
    def compile_cpp(self, source, workdir):
        ...
        # -include cassert: newer GCC trimmed transitive includes that 2021-era competitive code
        # relies on (assert via <bits/stdc++.h>); -std=gnu++17 enables the GNU extensions assumed.
    ...
    def run(self, cmd, stdin_text, time_limit, workdir):
        wrapped = ["/usr/bin/sandbox-exec", "-p", self._profile(workdir)] + cmd
        t0 = time.time()
        # errors="replace": buggy submissions can emit non-UTF-8 bytes; decode leniently
    ...
    def _limits(self, time_limit):
        ...
        def apply():
            ...
            try:
                ...
                pass
            # RLIMIT_AS intentionally NOT set: on macOS it falsely kills normal processes (huge
            # virtual reservations) instead of capping real memory; the wall-clock timeout bounds
            # runaway memory instead (see ASSUMPTIONS.md).
