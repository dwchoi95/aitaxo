...
PROMPT = (
    ...
    "- GE1.1 (Incorrect Algorithm) is the DEFAULT for a wrong overall approach or a wrong "
    "method/formula. Prefer GE1.1 unless the evidence clearly points to a more specific leaf.\n"
    "- Use a specific AE* leaf ONLY when the overall approach is correct but one concrete "
    "algorithmic step is implemented wrong (e.g. a wrong DP transition, a wrong base case, a "
    "missing graph transition). When unsure between GE1.1 and an AE* leaf, choose GE1.1.\n"
    "- GE1.2 (Misunderstanding Requirements): use ONLY with clear evidence the code solves a "
    "DIFFERENT problem than the one asked (misread the statement). A merely wrong answer is "
    "GE1.1, not GE1.2.\n"
